	...
	*/
	function datasetadd(){
		...
		if ($this->input->server('REQUEST_METHOD') === "POST") {
            ...
            $this->form_validation->set_rules('state', 'State', 'trim|required'); 
            $this->form_validation->set_rules('city[]', 'City', 'trim|required'); 
            $this->form_validation->set_rules('datacount', 'Dataset count', 'trim|required|numeric'); 
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                }else if(form_error('datacount')!='') {
                    $this->message = form_error('datacount');
                }
	...
	function importdatasetcsv(){
        ...
        public function payu_TransactionHistory() {  
            ...
            //    $this->app->message('Transaction have completed.', 'success'); 
                ...
                $data['history_Data']       = $this->common->getTransactionHistory();
        ...
        public function saveBlog(){
                ...
                if($this->input->server('REQUEST_METHOD')==="POST"){
                    ...
                    if(!empty($_FILES['blogPic']['name']) && $_FILES['blogPic']['size'] > 0){

//                        echo '<pre>';
//                        $destinatiofilenaem = FCPATH.'uploads/blogPic/';
//                        $imagePath = '/home/trivialworks060/Desktop/wordrobe.png';
//                        print_r($_FILES);
//                        
//                     //   $_getCropedImage = $this->common->resize_image($imagePath,$destinatiofilenaem);
//                     //   print_r($$_getCropedImage);
//                        
//                        
//                         $x=288; $y=202; // my final thumb
//                        $ratio_thumb=$x/$y; // ratio thumb
//
//                        list($xx, $yy) = getimagesize($imagePath); // original size
...
//                        }
//
//                        $ddd=   imagecopyresampled($thumb, $source, 0, 0, $xo_ini, $xy_ini, $x, $y, $xo, $yo);
//                        
//                        echo "ddddddd.<pre>";
//                        print_r($ddd);
//                        
...
//                        
                        ...
                        $config['upload_path'] = FCPATH.'uploads/blogPic/';  
                        ...
                        if($this->upload->do_upload('blogPic')){
                            ...
                            $image = base_url().'uploads/blogPic/'. $image1; 
                            chmod($this->upload->data()['full_path'],0777);
                             $insData['blogImage']       =   $image;
                        }else{
                            ...
                            redirect('admin/addBlog');
                                 ...
                                 redirect('admin/editBlog/'.$postData['blogID']);
                            }
                       }
                    }
                    
                        $blogTitle   = $this->form_validation->set_rules('blogTitle', 'Blog Title', 'trim|required|min_length[3]');     
                        $blogContent = $this->form_validation->set_rules('blogContent', 'Blog Contents', 'trim|required|min_length[3]');     
                    
                        if($this->form_validation->run()!=FALSE){
                            
                          
                            
                            $insData['blogHead']        = $postData['blogTitle'];
                            $insData['blogText']        = $postData['blogContent'];
                            $insData['status']          = ($postData['status'])?'1':0;
                           
                            $insData['blogCreatedTime'] =   date('Y-m-d H:i:s');
                           //   print_r($insData); die;
                            if($postData['blogID']!=''){
                                
                                $blogID                 =   $postData['blogID'];
                                $blogID  = $this->common->update('blog',array('blogID'=>$blogID),$insData);
                            }else{
                            
                            $blogID  = $this->common->insert('blog',$insData);
                            }
                            if($blogID){
                            redirect('admin/blog')  ;
                            }else{
                                     $this->app->message('There is error in insert process','error');    
                                     redirect('admin/addBlog')  ;    
                            }
                        }else{
                        
                            if(!empty(form_error('blogTitle'))){
                                 $this->message = form_error('blogTitle');    
                            }else{
                                 $this->message = form_error('blogContent');    
                            }
                    ...
                    $this->app->message('Invalid mrthod used.', 'error');
                    redirect('admin'); 
        ...
        public function manageBlog(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                $data['page_name']			= "Blog";
                $data['page_slug']			= 'blog';   
                $data['bloglist']                       = $this->common->_getList('blog','','blogID desc');
                $this->load->view('admin/common/header.php',$data);	 
                $this->load->view('admin/common/topheader.php',$data);
                $this->load->view('admin/common/sidebarMenu.php',$data);
	  ...
	  public function editBlog(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                $data['blog_data'] = $this->common->_getById('blog',array('blogID'=>$blogID));
          ...
          public function deleteBlog(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                if($blogID==''){
                $this->app->message("BlogID is required to delete a blog",'error')    ;
                }else{
                    $this->common->delete_by_id('blog',array('blogID'=>$blogID));
                    $this->app->message("Delete successfully ",'success')    ;
                }
      ...
      public function addTestimonial(){
                ...
                if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){
                $data['page_title'] 		= SITE_TITLE;
                $data['page_name']			= "Create Testimonial";
                $data['page_slug']			= 'testimonial';   
        ...
        public function saveTestimonial(){
                ...
                if($this->input->server('REQUEST_METHOD')==="POST"){
                    ...
                    $postData  =$this->input->post();
                        ...
                        $userName    = $this->form_validation->set_rules('userName', 'Username', 'trim|required|min_length[3]');     
                        $Content     = $this->form_validation->set_rules('Content', 'Testimonial Contents', 'trim|required|min_length[3]');     
                    
                        if($this->form_validation->run()!=FALSE){
                            
                          
                            
                            $insData['userName']                     = $postData['userName'];
                            $insData['testmonialDescription']        = $postData['Content'];
                            $insData['status']                       = ($postData['status'])?'2':'1';
                           
                            $insData['userCommentTime'] =   date('Y-m-d H:i:s');
                          
                            if($postData['testiID']!=''){
                                
                                $testiID                 =   $postData['testiID'];
                                $testID  = $this->common->update('testimonial',array('testmonialID'=>$testiID),$insData);
                            }else{
                            
                            $testID  = $this->common->insert('testimonial',$insData);
                            }
                            if($testID){
                            ...
                            if(!empty(form_error('userName'))){
                                 $this->app->message(form_error('userName'),'error');    
                            }else if(!empty(form_error('Content'))){
                                 $this->app->message(form_error('Content'), 'error') ;    
                            }
        ...
        public function manageTestimonial(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                $data['page_slug']			= 'testimonial';   
                $data['testlist']                       = $this->common->_getList('testimonial','','testmonialID desc');
                $this->load->view('admin/common/header.php',$data);	 
                $this->load->view('admin/common/topheader.php',$data);
                $this->load->view('admin/common/sidebarMenu.php',$data);
	  ...
	  public function editTestimonial(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                $data['test_data'] = $this->common->_getById('testimonial',array('testmonialID'=>$testID));
          ...
          public function deleteTestimonial(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                if($testID==''){
                $this->app->message("Testmonial ID is required to delete a Testimonial",'error')    ;
                }else{
                    if($this->common->delete_by_id('testimonial',array('testmonialID'=>$testID)))
                    $this->app->message("Delete successfully ",'success');
                }
        ...
        public function addQuestion(){
                ...
                if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){
                $data['page_title'] 		= SITE_TITLE;
                $data['page_name']			= "Create Question";
        ...
        public function saveQuestion(){
                ...
                if($this->input->server('REQUEST_METHOD')==="POST"){
                    ...
                    $postData  =$this->input->post();
                         $this->form_validation->set_rules('question_data', 'Question ', 'trim|required|min_length[3]');     
                         $this->form_validation->set_rules('answer_data', 'Answer', 'trim|required|min_length[3]');     
                    
                        if($this->form_validation->run()!=FALSE){
                            
                          
                            
                            $insData['questionText']        = $postData['question_data'];
                            $insData['answerText']        = $postData['answer_data'];
                            $insData['status']          = ($postData['status'])?'1':'0';
                           
                            $insData['createdAt'] =   date('Y-m-d ');
                           //   print_r($insData); die;
                            if($postData['questionID']!=''){
                                
                                $questionID             =   $postData['questionID'];
                                $questionID              = $this->common->update('question',array('questionID'=>$questionID),$insData);
                            }else{
                            
                                $questionID  = $this->common->insert('question',$insData);
                            }
                            if($questionID){
                                 redirect('admin/question')  ;
                                    ...
                                    if($postData['questionID']!=''){  
                                      $this->app->message('There is error in update process','error'); 
                                       redirect('admin/editQuestion/'.$postData['questionID']);    
                                 }else{
                                    $this->app->message('There is error in insert process','error'); 
                            ...
                            if(!empty(form_error('question_data'))){
                                  $this->app->message(form_error('question_data'),'error');     
                            }else if(!empty(form_error('answer_data')))
                             {
                                 $this->app->message(form_error('answer_data'),'error');    
                            }
         ...
         public function managequestion(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                $data['page_slug']			= 'question';   
                $data['questionlist']                       = $this->common->_getList('question','','questionID desc');
                $this->load->view('admin/common/header.php',$data);	 
                $this->load->view('admin/common/topheader.php',$data);
                $this->load->view('admin/common/sidebarMenu.php',$data);
	  ...
	  public function editQuestion(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                $data['question_data'] = $this->common->_getById('question',array('questionID'=>$questionID));
          ...
          public function deleteQuestion(){
            if(!empty($this->session->all_userdata()['userId']) && ($this->session->all_userdata()['userType'] =='1' || $this->session->all_userdata()['userType'] =='2' )){  
                ...
                if($questionID==''){
                $this->app->message("QuestionID is required to delete a Question",'error')    ;
                }else{
                    $this->common->delete_by_id('question',array('questionID'=>$questionID));
                    $this->app->message("Delete successfully ",'success')    ;
                }
